...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn,  muduo::net::Buffer *buffer,  muduo::Timestamp)
{
    ...
    //获取service对象
    auto it = m_serviceMap.find(service name);
    ...
    //获取method对象
    auto mit = it->second.m_methodMap.find(method name);
    ...
    {
        std::cout << "request parse error, content:" << args str << std::endl;
        return;
    }

    //生成rpc方法调用的响应response参数
...
void RpcProvider::sendRpcResponse(const muduo::net::TcpconnectionPtr& conn, google::protobuf::Message*response)
{
    ...
    {
        conn->send(response_str);    //response序列化成功，通过网络把RPC方法执行的结果发送给RPC调用方
    }
    else
    {
        std::cout << "serialize response_str error!" << std::endl;    //response序列化失败
    }
    conn->shutdown();            //默认HTTP的短链接服务，由RpcProvider主动断开连接
}
